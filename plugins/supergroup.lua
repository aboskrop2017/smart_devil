...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'اهلا بك عزيزي المطور 👋🏻 : @'..msg.from.username.."\nتم تفعيلي في المجموعة 🤖✅"
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = 'اي بعد احسن 😏 يلا جاو شعدي باقي هنا 😁😂 : '..msg.to.title
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local markdown = '*☺️🍃 Hi, *['..msg.from.first_name..']('..user..')'
  ...
  send_markdown(msg, get_receiver_api(msg), text, true, 'md')
end
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'اكلك لتزلك شنو حاطني تهين بيه كل شويه وسويت الغاء تفعيل 👞😂'
  end
  -- determine if table is empty
  if next(data[tostring(msg.to.id)]['moderators']) == nil then
    local text = '\nNo moderator in this group.'
   return text
  end
  local i = 1
...
local function DevPointTeam(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		  ...
		  local iDev1 = "اكلك حبي انته بيك شي 😒 مو قبل شويه فعلتني ادري هيه لزكة 😂"
		...
		if matches[1] == 'addbot' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
			  local iDev1 = "اهووووو 😒 يومية مفعلني بكروب خرب ربك 😱😋"
			  return send_large_msg(receiver, iDev1)
			end
		...
		if matches[1] == 'rembot' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'اكلك لتزلك شنو حاطني تهين بيه كل شويه وسويت الغاء تفعيل 👞😂', ok_cb, false)
			end
			print("SuperGroup "..msg.to.print_name.."("..msg.to.id..") removed")
			superrem(msg)
			rem_mutes(msg.to.id)
		...
		if matches[1] == 'addowner' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'addowner',
					msg = msg
			...
			elseif matches[1] == 'addowner' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local	get_cmd = 'addowner'
				local	msg = msg
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'addowner' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'addowner'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'addrem' then
			...
			elseif matches[1] == 'addrem' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'addrem'
      ...
      local markdown = '*☺️🍃 Hi, *['..msg.from.first_name..']('..user..')'
			...
			local about = markdown.."\n\n*Description has been set.*\n*Select the chat again to see the changes.*"
			send_markdown(msg, get_receiver_api(msg), about, true, 'md')
		end
...
local markdown = '*☺️🍃 Hi, *['..msg.from.first_name..']('..user..')'
			...
			local rules = markdown..'\n\n*Please send the new group photo now*'
			send_markdown(msg, get_receiver_api(msg), rules, true, 'md')
		end
		...
		if matches[1] == 'lg' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1] == 'ug' and is_momod(msg) then
			local target = msg.to.id
			     if matches[2] == 'all' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == 'lg' and is_owner(msg) then
		  local user = 'https://telegram.me/'..(msg.from.username or msg.from.first_name)
      local markdown = '['..msg.from.first_name..']('..user..')' 
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					mute(chat_id, msg_type)
					    local text = '*Audio has been muted* 🎵🔐\n👮 *Order by* :️ '..markdown
					    send_markdown(msg, get_receiver_api(msg), text, true, 'md')
				else
					    local text1 = '*Audio is already muted* 🎵🔐\n👮 *Order by* :️ '..markdown
					    					    send_markdown(msg, get_receiver_api(msg), text1, true, 'md')
				end
			end
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local pht = '*Photo has been muted* 🎡🔐\n👮 *Order by* :️ '..markdown
   send_markdown(msg, get_receiver_api(msg), pht, true, 'md')
				else
					local pht1 = '*Photo is already muted* 🎡🔐\n👮 *Order by* :️ '..markdown
send_markdown(msg, get_receiver_api(msg), pht1, true, 'md')
			...
			local msg_type = 'Video'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local vid = '*Video has been muted* 🎥🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), vid, true, 'md')
				else
					local vid1 = '*Video is already muted* 🎥🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), vid1, true, 'md')
				end
			end
			...
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local gif = '*Gifs has been muted* 🎆🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), gif, true, 'md')
				else
					local gif1 = '*Gifs  is already muted* 🎆🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), gif1, true, 'md')
				end
			end
			...
			local msg_type = 'Documents'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local doc = '*documents has been muted* 📂🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), doc, true, 'md')
				else
					local doc1 = '*documents  is already muted* 📂🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), doc1, true, 'md')
				end
			end
			...
			local msg_type = 'Text'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local txt = '*Text has been muted* 📝🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), txt, true, 'md')
				else
					local txt1 = '*Text is already muted* 📝🔐\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), txt1, true, 'md')
				end
			end
			if matches[2] == 'all' then
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					local all = '*Mute all has been enabled* 🔕\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), all, true, 'md')
				else
					local all1 = '*Mute all is already enabled* 🔕\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), all1, true, 'md')
				end
			end
		end
		if matches[1] == 'ug' and is_momod(msg) then
      ...
      local markdown = '['..msg.from.first_name..']('..user..')' 
			...
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					local uaud = '*Audio has been unmuted* 🎵🔓\n👮 *Order by* :️ '..markdown
						send_markdown(msg, get_receiver_api(msg), uaud, true, 'md')
				else
					local uaud1 = '*Audio is already unmuted* 🎵🔓\n👮 *Order by* :️ '..markdown
						send_markdown(msg, get_receiver_api(msg), uaud1, true, 'md')
				end
			end
			if matches[2] == 'photo' then
			local msg_type = 'Photo'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					local pht2 = '*Photo has been unmuted* 🎡🔓\n👮 *Order by* :️ '..markdowne
						send_markdown(msg, get_receiver_api(msg), pht2, true, 'md')
				else
					local pht3 = '*Photo is already unmuted* 🎡🔓\n👮 *Order by* :️ '..markdown
						send_markdown(msg, get_receiver_api(msg), pht3, true, 'md')			
				end
			end
			if matches[2] == 'video' then
			local msg_type = 'Video'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					local vid2 = '*Video has been unmuted* 🎥🔓\n👮 *Order by* :️ '..markdown
									send_markdown(msg, get_receiver_api(msg), vid2, true, 'md')
				else
					local vid3 = '*Video is already unmuted* 🎥🔓\n👮 *Order by* :️ '..markdown
									send_markdown(msg, get_receiver_api(msg), vid3, true, 'md')
				end
			end
			if matches[2] == 'gifs' then
			local msg_type = 'Gifs'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					local gif2 = '*Gifs has been unmuted* 🎆🔓\n👮 *Order by* :️ '..markdown
									send_markdown(msg, get_receiver_api(msg), gif2, true, 'md')
				else
					local gif3 = '*Gifs is already unmuted* 🎆🔓\n👮 *Order by* :️ '..markdown
									send_markdown(msg, get_receiver_api(msg), gif3, true, 'md')
				end
			end
			if matches[2] == 'doc' then
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					local doc2 = '*documents has been unmuted* 📂🔓\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), doc2, true, 'md')
				else
					local doc3 = '*documents is already unmuted* 📂🔓\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), doc3, true, 'md')
				end
			end
			if matches[2] == 'text' then
			local msg_type = 'Text'
				if is_muted(chat_id, msg_type..': yes') then
					...
					unmute(chat_id, msg_type)
					local txt2 = '*Text has been unmuted* 📝🔓\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), txt2, true, 'md')
				else
					local txt3 = '*Text is already unmuted* 📝🔓\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), txt3, true, 'md')
				end
			end
			if matches[2] == 'all' then
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					local all3 = '*Mute all has been disabled* 🔔\n👮 *Order by* :️ '..markdowne
					send_markdown(msg, get_receiver_api(msg), all3, true, 'md')
				else
					local all4 = '*Mute all is already disabled* 🔔\n👮 *Order by* :️ '..markdown
					send_markdown(msg, get_receiver_api(msg), all4, true, 'md')
				end
			end
		end
